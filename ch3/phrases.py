...
from ch1.removing_stopwords import read_in_csv

stopwords_file = "ch1/stopwords.csv"
stopwords = read_in_csv(stopwords_file)
yelp_reviews_file = "ch3/yelp-dataset/review.json"
...
def main():
    ...
    #write_text_to_file(text, "ch3/all_text.txt")
    ...
    #fdist = create_and_save_frequency_dist(flat_word_list, "ch3/fdist.bin")

    
    fdist = pickle.load(open("ch3/fdist.bin", "rb"))
    #write_text_to_file(str(fdist.most_common()[:1000]), "ch3/most_frequent_phrases.txt")
    print(fdist.most_common()[:1000])

    #model = create_and_save_word2vec_model(all_sentence_words, "ch3/phrases.model")
    model = gensim.models.Word2Vec.load("ch3/phrases.model")
